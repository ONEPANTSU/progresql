...
import {
  ...
  ServerNotification,
  ToolResultPayload,
} from '@/features/agent-chat/AgentService';
import { handleToolCall } from '@/features/agent-chat/toolHandler';
import type { PendingApproval, SqlDangerLevel } from '@/features/agent-chat/ui/ToolApprovalDialog';
...
import {
  ...
  saveSecurityMode,
  SecurityMode,
} from '@/shared/lib/secureSettingsStorage';
import { isSubscriptionActive, fetchUsage } from '@/features/auth/auth';
...
export interface AgentContextValue {
  ...
  /** Reset "accept always" auto-approval (call on new chat, mode/model change) */
...
export const AgentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const value = useMemo<AgentContextValue>(() => ({
    ...
    pendingApproval,
    resetAutoApproval,
  }), [connectionState, connectionPhase, isAuthError, connect, disconnect, sendRequest, cancelRequest, sessionId, error, backendUrl, setBackendUrl, model, setModel, autocompleteModel, setAutocompleteModel, autocompleteEnabled, setAutocompleteEnabled, securityMode, setSecurityMode, setSafeMode, sendAutocomplete, cancelAutocomplete, usage, refreshUsage, lastNotification, pendingApproval, resetAutoApproval]);
