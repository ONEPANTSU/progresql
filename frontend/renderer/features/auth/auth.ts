import { AuthUser, SubscriptionWarning } from '@/shared/types';
...
export async function fetchPrices(): Promise<import('@/shared/types').PricesResponse> {
  const baseUrl = getBackendUrl();
  const token = getAuthToken();
...
export async function fetchBalance(): Promise<import('@/shared/types').BalanceInfo> {
  const baseUrl = getBackendUrl();
  const token = getAuthToken();
...
export async function fetchBalanceHistory(
  limit = 20,
  offset = 0,
): Promise<{ transactions: import('@/shared/types').BalanceTransaction[]; total: number }> {
  const baseUrl = getBackendUrl();
  const token = getAuthToken();
...
export async function fetchUsage(): Promise<import('@/shared/types').UsageInfo> {
  const baseUrl = getBackendUrl();
  const token = getAuthToken();
...
export async function fetchQuota(): Promise<import('@/shared/types').QuotaInfo> {
  const baseUrl = getBackendUrl();
  const token = getAuthToken();
...
export async function fetchUsageHistory(
  limit = 20,
  offset = 0,
): Promise<import('@/shared/types').UsageHistoryResponse> {
  const baseUrl = getBackendUrl();
  const token = getAuthToken();
...
export async function fetchModelPricing(): Promise<import('@/shared/types').ModelPricingResponse> {
  const baseUrl = getBackendUrl();
...
export async function fetchModels(): Promise<{ models: import('@/shared/types').ModelInfo[] }> {
  const baseUrl = getBackendUrl();
...
export function getSubscriptionWarning(user: AuthUser | null): SubscriptionWarning {
  ...
  // Check trial expiry (for free/trial plans)
  if (user.trialEndsAt) {
